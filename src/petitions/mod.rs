...
pub fn handle_error_petition_log(error: &ErrorPetition) {
    match error {
        ErrorPetition::StatusResponseNotValid(number) => log::error!("Failed response with status: {number}"),
...
fn validate_status_response(response: &Response)-> Result<(), ErrorPetition>{ 
