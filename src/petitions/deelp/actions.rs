...
pub fn build_client_to_deepl<'a>(api_key: &'a str) -> Client {
    ...
    header.append(
        AUTHORIZATION,
        format!("{} {}", NAME_API_KEY, api_key)
            .parse()
            .expect("Error building api key header value"),
    );

    Client::builder()
        .default_headers(header)
        .build()
        .expect("Error building client to deepl")
}
...
pub async fn send_petition<'a, 'b>(
    ...
    let result_response = client.post(URL_FREE_DEEPL).form(&params).send().await;
